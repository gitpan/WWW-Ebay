...
sub signin
  ...
  DEBUG_FUNC && print STDERR " + Ebay::Session::signin($sUserID)\n";
  print STDERR " + signin($sUserID,$sPassword)\n" if DEBUG_FETCH;
  if (! exists($hssPasswords{$sUserID}))
    {
    # Get the sign-in page and parse it:
    print STDERR " +   fetching ebay sign-in page...\n" if DEBUG_FETCH;
...
sub fetch_any_ebay_page
  ...
  print STDERR " + in fetch_any_ebay_page, oReq is $oReq\n" if DEBUG_FETCH;
  my $ref = ref $oReq;
 ...
 REQUEST_READY:
  ...
  if ($sName ne '')
    {
    print STDERR " +   the HTTP::Request for $sName is $sReq" if DEBUG_FETCH;
    } # if
  $self->response($self->user_agent->request($oReq));
  my $sRes = $self->response->as_string;
  DEBUG_FETCH && print STDERR " +   the HTTP::Response for $sName is ==========$sRes==========";
  my $sURLprev = '';
 OBJECT_MOVED:
  while ($self->response->code == 302)
    {
    print STDERR " +     server says: Object Moved\n" if DEBUG_FETCH;
    ...
    $self->cookie_jar->add_cookie_header($oReq);
    print STDERR " +   the new HTTP::Request for $sName is ", $oReq->as_string if DEBUG_FETCH;
    $sReq .= $oReq->as_string;
    $self->response($self->user_agent->request($oReq));
    } # while
 META_REFRESH:
  while (! $iIgnoreRefresh && ($self->response->content =~ m!<meta\s+http-equiv="Refresh"\s+content="\d+;\s+url\s*=\s*([^"]+)">!i))
    ...
    $sURL = $1;
    $sURL =~ s!&amp;!&!g;
    print STDERR " +     server says: Meta-Refresh to $sURL\n" if DEBUG_FETCH;
    $oReq = GET $sURL;
    $oReq->referer($sURLprev);
    $self->cookie_jar->add_cookie_header($oReq);
    print STDERR " +   the new HTTP::Request for $sName is ", $oReq->as_string if DEBUG_FETCH;
  ...
  if ($sPage =~ m!"SignInForm"!)
    ...
    if (DEBUG_FETCH && ($sName ne '') && (open PAGE, '>', $fname1))
      ...
      print PAGE $sPage;
      close PAGE;
      print STDERR " + eBay GET $sName saved in $fname1\n" if DEBUG_FETCH;
      } # if
    ...
    $oReq->referer($sURLprev);
    $sURLprev = $sURL;
    print STDERR " +   CLICK is ", Dumper($oReq) if DEBUG_FETCH;
    print STDERR " + submitting password to ebay...\n" if DEBUG_FETCH;
    goto REQUEST_READY;
    ...
    $oReq->referer($sURLprev);
    $sURLprev = $sURL;
    print STDERR " + giving adult-consent to ebay...\n" if DEBUG_FETCH;
    print STDERR " +   CLICK is ", Dumper($oReq) if DEBUG_FETCH;
    goto REQUEST_READY;
    }
  else
    ...
    if (DEBUG_FETCH && ($sName ne '') && (open PAGE, '>', $fname))
      ...
      print PAGE $sPage;
      close PAGE;
      print STDERR " + eBay GET $sName saved in $fname\n" if DEBUG_FETCH;
      } # if
    } # else
  return $sPage;
  } # fetch_any_ebay_page
 ...
 SELLING_COLUMN:
      foreach my $sCol (@asColumns)
        ...
        if ($sCol eq 'price')
          ...
          print STDERR " +   end date: raw ==$sDateRaw== cooked ==$sDate== date==$date==\n" if DEBUG_SELLING;
          }
...
sub sold_auctions
 ...
 PARSE_SOLD_SECTION:
  while (1)
    ...
    if (ref $oA)
      {
      DEBUG_SOLD && _debug(" +   found <SPAN> for SOLD section: ", $oA->as_HTML, "\n");
      $oA = $oA->parent;
      my $s = $oA->as_text;
      $iCount = -1;
      if ($s =~ m!\(\s*(\d+)\s+ITEM!i)
        {
        $iCount = $1;
        DEBUG_SOLD && _debug(" +   there should be $iCount sold auctions\n");
        } # if
      } # if
    else
      {
    ...
    # print STDERR " +   sold <TABLE> is ==", $oTable->as_HTML, "==\n" if DEBUG_SOLD;
    my @aoTR = $oTable->look_down(_tag => 'tr',
                                  bgcolor => '#f4f4f4',
                                 );
 SOLD_TR:
    while (my $oTR = shift @aoTR)
      ...
      $oTD = shift @aoTD;
      DEBUG_SOLD && _debug(" +   quantity <TD> ==", $oTD->as_HTML, "==\n");
      ...
      DEBUG_SOLD && _debug(" +     <TD> containing EndPrice ==", $oTD->as_HTML, "==\n");
      $s = $oTD->as_text;
      print STDERR " +     raw End Price is ==$s==\n" if DEBUG_SOLD;
      $s =~ tr!.0123456789!!dc;
      # Convert dollars to cents:
      my $iBidCents = int((0.005 + $s) * 100);
      print STDERR " +     Bid Cents is ==$iBidCents==\n" if DEBUG_SOLD;
      ...
      # will be '--'.
      $oTD = shift @aoTD;
      DEBUG_SOLD && _debug(" +   <TD> of total price ==", $oTD->as_HTML, "==\n");
      $s = $oTD->as_text || '';
      ...
      if ($s eq '--')
        ...
        print STDERR " +     Total Cents is ==$iTotalCents==\n" if DEBUG_SOLD;
      ...
      DEBUG_SOLD && _debug(" +   <TR> of next row ==", $oTR->as_HTML, "==\n");
      $oA = $oTR->look_down(_tag => 'a');
      next SOLD_TR unless ref $oA;
      DEBUG_SOLD && _debug(" +   <A> of title ==", $oA->as_HTML, "==\n");
      my $sTitle = $oA->as_text;
      $sTitle =~ s![\s\t\r\n]+\Z!!;
      $oWEL->title($sTitle);
      my $sURL = $oA->attr('href');
...
sub unsold_auctions
 ...
 PARSE_UNSOLD_SECTION:
  while (1)
    ...
    if (ref $oAunsold)
      {
      print STDERR " +   found <A> for UNSOLD section: ", $oAunsold->as_HTML, "\n" if DEBUG_UNSOLD;
      my $s = $oAunsold->as_text;
      $s =~ m!\(\s*(\d+)\s+Items?!;
      $iCount = $1 || 0;
      print STDERR " +   there should be $iCount UNSOLD auctions\n" if DEBUG_UNSOLD;
      } # if
    if ($iCount <= 0)
      {
    ...
    print STDERR " +   ancestor <TABLE> is ==", $oTable->as_HTML, "==\n" if DEBUG_UNSOLD;
    ...
    print STDERR " +   n-th TABLE sibling of ancestor <TABLE> is ==", $oTable->as_HTML, "==\n" if DEBUG_UNSOLD;
    my @aoTR = $oTable->look_down('_tag' => 'tr');
 TR:
    while (my $oTR = shift @aoTR)
      {
      ...
      # rows; one row has the auction title, the next row has all the
      # details.
      print STDERR " +   <TR> containing unsold auction title ==", $oTR->as_HTML, "==\n" if DEBUG_UNSOLD;
      my $oA = $oTR->look_down('_tag' => 'a');
      next TR unless ref $oA;
      # Make sure this is really an auction title/link:
      next TR unless defined($oA->attr('href'));
      ...
      $oTD = $oTD->left;
      next TR unless ref $oTD;
      print STDERR " +     <TD> containing Item# ==", $oTD->as_HTML, "==\n" if DEBUG_UNSOLD;
      $s = $oTD->as_text;
